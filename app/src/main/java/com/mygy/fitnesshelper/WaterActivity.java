...
public class WaterActivity extends AppCompatActivity {
    ...
    private User user;
    private TextView waterDone;
    private TextView waterNeed;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ImageButton settingsBtn = findViewById(R.id.water_settings);
        settingsBtn.setOnClickListener(v -> {
            User.showChangeTagretWindow(this,"воды",user.getWaterTarget(), inp -> {
                user.setWaterTarget((int)inp);
                updateInfoData();
            });
        });

        waterDone = findViewById(R.id.steps_stepsDone);
        waterNeed = findViewById(R.id.steps_stepsNeed);
        dateTV = findViewById(R.id.steps_date);
        ...
        addSteps.setOnClickListener(v -> {
            User.showAddValueWindow(this,"воду",(date,val) -> {
                user.addWater(date,(int)val);
                updateInfoData();
                updateBarData();
            });
        });
    ...
    private void updateInfoData(){
        ...
        dateTV.setText(dateFormat.format(selectedDate));
        waterDone.setText(Integer.toString(user.getWaterAtDate(selectedDate)));
        waterNeed.setText(Integer.toString(user.getWaterTarget()));
    }
    private void updateBarData(){
        ...
        for(int i = 0; i < 7; i++){
            ...
            barEntries.add(new BarEntry(i,user.getWaterAtDate(day.getTime())));
        }

        BarDataSet barDataSet = new BarDataSet(barEntries,"water count");
